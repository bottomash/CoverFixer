...
- 管理员打开单个剧集详情页后，可以在右上角“三点”菜单中执行“刷新 TMDB 封面”。该菜单隐藏“随机播放”“添加到合集”“添加到播放列表”“收藏”“标记已播放”“下载到”“转换”和原“删除”命令，删除按钮改为与“三点”菜单平级的独立按钮。
- 菜单命令自动使用当前 Series Item ID，只使用 TheMovieDb 候选图替换当前主封面，不锁定元数据。
- 菜单通过运行时扩展 Emby Web 的 `shortcuts.js` 立即注册，不修改 Emby Web 的磁盘文件；从其他页面首次进入剧集详情时也会直接应用精简后的菜单。
